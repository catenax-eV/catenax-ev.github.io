...
defining the semantics of the different aspects of an asset that is represented via a digital twin.
Aspect models are machine readable and other artifacts can be derived from it.
The Aspect Model is the common basis for the http/REST API for accessing data via the standardized digital twin API of the Asset Administration Shell.
...
![SAMM 2.2.0](./assets/CX-0003-aspect-meta-model.jpg "Predefined SAMM Objects for Aspect Model Definition (Version 2.2.0)")
...
Every semantic model used in any of the Catena-X Standards **MUST** have a unique identifier conformant to the meta model identifiers definition in [SAMM](#2-semantic-aspect-meta-model-samm).
...
Example for a unique identifier for an Aspect Model \"Example\":

```
urn:samm:io.catenax.example:1.0.0#Example
```

The [Catena-X Governance Process for the Creation and Standardization of Semantic Models](https://catenax-ev.github.io/working-model/process-structure/process-semantic-models) for new, updated or deprecated Aspect Models **MUST** be followed.
...
```
urn:samm:io.catenax.example:3.0.0#Example
...
- [SAMM Semantic Aspect Meta Model, Version 2.2.0](https://eclipse-esmf.github.io/samm-specification/2.2.0/index.html).
