---
sidebar_position: 8
title: Appendix
---

## List of abbreviations

| Abbreviation | Designation                                                                                                            |
|--------------|------------------------------------------------------------------------------------------------------------------------|
| API          | Application Programming Interface                                                                                      |
| BPDM         | Business Partner Data Management                                                                                       |
| BPN          | Business Partner Number                                                                                                |
| CAB          | Conformity Assessment Body                                                                                             |
| CAC          | Conformity Assessment Criteria                                                                                         |
| CAF          | Conformity Assessment Framework                                                                                        |
| CMC          | Certification Management Center                                                                                        |
| CX           | Catena-X Automotive Network e.V., the association                                                                      |
| EDC          | Eclipse Dataspace Connector                                                                                            |
| ESS          | Environmental and Social Standards                                                                                     |
| IAM          | Identity and Access Management                                                                                         |
| ID           | Identification                                                                                                         |
| IDSA         | International Data Spaces Association                                                                                  |
| IDW          | Institut der Wirtschaftsprüfer in Deutschland e.V. [Institute of Public Auditors in Germany, Incorporated Association] |
| IoT          | Internet of Things                                                                                                     |
| IPE          | Information Produced/Provided by Entity                                                                                |
| ISO          | International Organization for Standardization                                                                         |
| OSIM         | Online control and simulation                                                                                          |
| PCF          | Product Carbon Footprint                                                                                               |
| PLM          | Product Lifecycle Management                                                                                           |
| PURIS        | Predictive Unit Real-Time Information Service                                                                          |
| R&D          | Research and Development                                                                                               |
| SD           | Self Description                                                                                                       |
| SDE          | Simple Data Exchanger                                                                                                  |
| SUS          | Sustainability                                                                                                         |
