...
Catena-X publishes **one [major](./how-life-cycle-management.md#major-releases)** and **one [minor](./how-life-cycle-management.md#minor-releases)** ecosystem release per year. This means that each ecosystem release can contain two quarterly releases of Catena-X - including standards and normative documents - as well as the corresponding two Tractus-X releases, which provide assets such as KITs and reference implementations.

Tractus-X will provide quarterly releases including **two major** and **two minor** releases per year. To ensure backward compatibility in the future, versioning and passing of predefined test cases of each individual artifact is mandatory.

The ecosystem releases follow a naming convention (NameVer). The release for Catena-X and Tractus-X follow calendar versioning (see [CalVer](https://calver.org/)), whereas their artifacts such as feature, reference implementations, and KITs follow semantic versioning (see [SemVer](https://semver.org/)).

![Overview Life Cycle Management](./assets/overview-life-cycle-management.png)
*Overview Life Cycle Management*

The **Catena-X release** includes binding and certification-relevant artifacts such as:

- Standards (e.g., semantic models, API specifications)
- Normative documents

The **Tractus-X release** provides:

- Open-source products (services, applications, helm charts)
- KITs

Commercial or self-developed solutions may follow individual release cycles but **must comply with Catena-X standards** to be deployable.  

Open-source products must comply with the [Tractus-X Release Guidelines (TRGs)](https://eclipse-tractusx.github.io/docs/release/) and pass integration testing to be included in quarterly Tractus-X releases. Critical issues (e.g., security vulnerabilities) may trigger hotfix releases.

## Categories of Versioning

Catena-X defines the type of changes. Each release cycle consists of a major and a minor release.

- **Major**
  - New functionality **MAY** be added, but not break existing functionality.
  - Existing functionality **MUST** continue to work.
  - Deprecation of existing functionality **MAY** be announced for the time span of a major release.
  - Already deprecated functionality **MAY** be removed (after maintenance period).
- **Minor**
  - New functionality **MAY** be added, but not break existing functionality.
  - Existing functionality **MUST** continue to work.
...
Major releases represent significant updates that may include new functionalities or architectural changes. They occur at most once per year and establish the baseline for compatibility and interoperability within the ecosystem.

Major changes can introduce new functionality that is not compatible with the existing functionality. The major version before has to continue to work. A disruption is not allow in the dataspace, because the old major version continues to work and the new major version is available in parallel.

#### Backwards compatibility

Compatibility between the releases is one of the cornerstones of a stable network. There are major versions with major changes, but such changes have to work with the grace period of the maintenance version in mind. This means that the evolution is ensured by following the approach of two major versions available at the same time.
...
In addition to the major changes, there are three different categories of major changes. The examples mentioned can fall into this category. Depending on the changes, the component in the example may also fall into a different category:
...
![Differences from major changes](./assets/impact-on-the-network.png)
*Differences from major changes*

### Minor releases

Minor releases introduce incremental improvements or feature enhancements delivered between major releases. New functionality **MAY** be added, provided it does not alter or disrupt existing behavior. All existing functionality **MUST** remain fully operational and backward compatible.

### Patches

Patches, on the other hand, can be introduced independently to fix bugs or security issues.

It can be problematic to fix bugs or security issues within a major or minor version, because the timeframe is too long. Therefore bugs shall be fixed independently of the release schedule, i.e. if a bugfix is required, it shall be done. A bug is a problem that is not changing the behavior, i.e. the behavior according to the standard is not changed.
...
All releases - major or minor as well as patches — must comply with the applicable Catena-X standards to ensure seamless operation across the dataspace.
...
The Catena-X data space aims to support a parallel phase of two major versions (each valid for a minimum of 12 months):
...
All participants doing changes to network communication or inter-company have to ensure one of the following variants to allow a smooth upgrade process:
