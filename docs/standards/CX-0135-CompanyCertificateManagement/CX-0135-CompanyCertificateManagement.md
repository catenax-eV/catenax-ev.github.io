...
- Data Provider wants to create, modify, withdraw a certificate.
- Data Consumer wants to be notified about all changes to a certificate.
- Data Consumer wants to discover a published certificate.
- Data Consumer wants to request a certificate from a specific Data Provider.
- Data Consumer wants to provide Data Provider with feedback on a certificate.
...
- The `Certificate Exchange` ([Section 2.1](#21-certificate-exchange)) — a single, correlatable interaction in which one
  certificate is delivered from a Data Provider to a Data Consumer and its outcome is reported back. A
...
A `Certificate Exchange` represents one end-to-end interaction between a Data Provider and a Data Consumer
...
A `Certificate Exchange` may be opened by the Data Consumer or Data Provider:

- **Consumer-Initiated:** the Data Consumer opens the exchange by requesting a certificate. The
  Data Provider assigns the `exchangeId` (together with the `certificateId` and `revision`) and returns them in
  ...
  correlatable.
- **Provider-Initiated:** the Data Provider opens the exchange when a certificate is already available (e.g.,
...
1. **Fulfillment (Provider-owned):** The Data Provider works to make the certificate available.
2. **Acceptance (Consumer-owned):** The Data Consumer retrieves and processes the certificate and reports the
   outcome.
...
|---------------------------|--------------------|-------------|----------|----------|-----------------------------------------------------------------------------------------------------------------------|
| `REQUESTED`               | Consumer-Initiated | Fulfillment | Provider | No       | The exchange was opened by the Data Consumer; the Data Provider has not yet acted.                                    |
| `ACKNOWLEDGED`            | Consumer-Initiated | Fulfillment | Provider | No       | The Data Provider accepted the request and began preparing the certificate.                                           |
| `CERTIFICATION_REQUESTED` | Consumer-Initiated | Fulfillment | Provider | No       | The Data Provider submitted the request to an external certification authority and is awaiting issuance. *(Optional)* |
| `FULFILLED`               | Consumer-Initiated | Fulfillment | Provider | No       | The certificate is prepared and available for retrieval and the Data Consumer was notified. Hand-off point.           |
| `DECLINED`                | Consumer-Initiated | Fulfillment | Provider | Yes      | The Data Provider declined the request (a business decision; e.g. the certificate type is not offered).               |
| `FAILED`                  | Consumer-Initiated | Fulfillment | Provider | Yes      | The Data Provider could not produce a valid certificate (a business error; e.g. the certificate is invalid).          |
| `RETRIEVED`               | -                  | Acceptance  | Consumer | No       | The Data Consumer fetched the certificate and is processing it. *(Optional — see the note below.)*                    |
| `ACCEPTED`                | -                  | Acceptance  | Consumer | Yes      | The Data Consumer accepted the certificate.                                                                           |
| `REJECTED`                | -                  | Acceptance  | Consumer | Yes      | The Data Consumer did not accept the certificate content (a business decision).                                       |
| `ERRORED`                 | -                  | Acceptance  | Consumer | Yes      | The Data Consumer found the certificate to be in error (a business error; e.g. the certificate is invalid).           |

##### 2.1.3.1 Consumer-Initiated

`REQUESTED` is instantaneous and internal to the Data Provider; it is never reported on the wire. The first
Fulfillment status a Data Consumer observes is the one carried in the request response (see
[Section 3.2.1](#321-certificate-request)), which is `ACKNOWLEDGED` or a later state. A provider that can satisfy a
...
`RETRIEVED` is a non-terminal acknowledgment that the Data Consumer has fetched the certificate and is
evaluating it; the Data Consumer **MAY** report it as a delivery receipt but is not required to.
...
The terminal acceptance verdicts remain consumer-owned in both cases. If the Data Consumer never provides any feedback, 
the terminal state is never reached for that exchange; it remains in `FULFILLED` indefinitely.
...
Data Provider accepts a Data Consumer's request and produces the certificate only later (see
...
applies to. On the full certificate record these are carried as structured `certifiedLocations` array (each with an 
optional per-location area of application; see [Section 3.2.2](#322-certificate-retrieval)); lighter projections such 
as query results and lifecycle events carry only the bare BPNs as `certifiedLocationBpns`. The location set is a static 
property of the certificate; a certificate that would cover a different set of locations is a distinct certificate. 
Be aware of effects on `revision` [Section 4.2.10](#4210-revision).
...
|-------------|----------|-----------------------------------------------------------------------------------------------------------------------------|
| `CREATED`   | No       | The certificate was first published under a new `certificateId`, establishing its initial version (`revision` 1).           |
...
| `WITHDRAWN` | Yes      | The Data Provider withdrew (removed) the certificate; it is no longer available. Terminal.                                  |
...
Of the publication events, only `CREATED` may open a new `Certificate Exchange`—whether pushed by the Data Provider or 
discovered and requested by the Data Consumer. `MODIFIED` and `WITHDRAWN` do not open an exchange: a modification 
revises the certificate in place under the same `certificateId` without initiating a new delivery, and a withdrawal 
ends the certificate's availability. Consistent with [Section 2.1.5](#215-relationship-to-the-certificate-lifecycle), 
a lifecycle transition never alters an existing `Certificate Exchange`. 
These transitions are communicated to Data Consumers as certificate lifecycle events (see
...
|----------------|-----------------------------------------------------------------|-------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| cx-taxo:CCMAPI | cx-taxo:CompanyCertificateManagementConsumerApi                 | 4.0         | Offers *Certificate Consumer API* according to [Section 3.1](#31-certificate-consumer-api) for receiving notifications on certificate lifecycle events and providing feedback on the status of provided certificates.                                                                                  |
...
- ```{ "dct:subject": { "@id": "cx-taxo:CompanyCertificateManagementConsumerApi" }, "cx-common:version": "4.0" }```,
...
The Certificate Consumer API enables a Data Consumer to receive notifications from Certificate
Provider (as certificate lifecycle events, see [Section 2.2](#22-certificate-lifecycle)) and provide certificate
acceptance status for open exchanges (see [Section 3.1.2](#312-certificate-acceptance-status-query)):

This API is optional; a Data Consumer that does not wish to receive notifications or provide certificate
acceptance status updates is not required to implement it. A Data Consumer that does implement it **MUST**
adhere to the following normative requirements (as well as further requirements defined in this section):
- The Data Consumer signals to the Data Provider the existence of the API by the availability of the
  data asset. If no data asset is available, the Data Provider is relieved of their obligation to push
  notifications. If the Data Consumer wishes to receive notifications, they **MUST** provide the data asset.
- If the data asset exists, the Data Consumer **MUST** serve all specified endpoints and implement at least one
  of them, but **MAY** return `HTTP 501` for endpoints it does not wish to implement.
  For example, a consumer that wants to receive notifications but does not want to receive acceptance status updates
  could return `HTTP 501` for the acceptance status endpoint.
- Reference, for the property [subject], exactly one of the two Certificate Consumer API subjects: 
  `cx-taxo:CompanyCertificateManagementConsumerApi` (lifecycle notifications carry document references only) 
  OR 
  `cx-taxo:CompanyCertificateManagementConsumerEmbeddedDocumentApi` 
  (lifecycle notifications additionally embed inline documents, see Section [3.1.1](#311-certificate-lifecycle-events)).
...
A Data Provider **MUST** push notifications to a Data Consumer that implements this API (and therefore
**MUST** check for its existence), and **MUST** follow the requirements defined in this section when doing so.
This obligation applies only to the endpoints the Data Consumer actually serves: if the Data Consumer 
returns HTTP 501 for an endpoint (see above), the Data Provider is relieved of the obligation to push to that endpoint.
Polling ([Section 3.1.2](#312-certificate-acceptance-status-query)) remains available as a fallback and is not a 
substitute for the push obligation.
...
**Example Certificate Consumer API Asset:**
...
The Data Provider **MUST** push notifications for all states from
...
The Data Provider pushes a `CREATED` notification (this would always be the case), and a few days later a
`MODIFIED` notification, because an error in the certificate metadata had to be corrected. A few years later the
Data Provider could push a `WITHDRAWN` notification, because the certificate is no longer valid. This last step is not
mandatory, because a Data Provider could decide to still keep the certificate available, even if it is expired.
...
| `exchangeId`  | String | **MANDATORY** for `CREATED` | Identifier of the exchange when opened by the `CREATED` event; absent otherwise.                                                                |
| `certificate` | Object | **MANDATORY**               | The certificate record (see [Section 4](#4-aspect-models)). How much of it is populated depends on the Data Consumer's API subject (see below). |

___
# TODO: Check against openAPI document for certificate property #
___

The Data Provider **MUST** also push notifications for all provider owned terminal states from
...
Data Consumers that want to receive also certificate payload with their lifecycle events, they can do
the following way: When registering the asset as described in the previous section [3.1](#31-certificate-consumer-api),
the Data Consumer **MUST** add the `property` to the asset:
...
A Data Consumer that wants the full certificate record and document content delivered inline with its lifecycle
...
**complete** certificate record as defined in [Section 3.2.2](#322-certificate-retrieval),
**and MUST** include the `documents` array. Unlike the retrieval response, where document content
...
When `status` is `DECLINED` or `FAILED`, the `data` **MUST** also include non-empty `errors` array (e.g.,
`"errors": [ { "message": "Certificate type not offered for the requested location" } ]`), 
as defined in the openAPI definition for `status` `REJECTED` from [Section 3.2.3](#323-certificate-acceptance).
...
The Certificate Provider API enables Data Providers to accept certificate requests, report request fulfillment
status, serve certificate data, answer certificate queries, and receive acceptance status from Data Consumers.
...
- reference the version of the API according to the OpenAPI specification for the property
  [[version]](https://w3id.org/catenax/ontology/common#version): `4.0`

**Example Certificate Provider API Asset:**
...
The Data Provider **MUST** respond with request states according to
...
The `/certificates/{id}` endpoint **MUST** be based on [CX-0002](#cx-0002), Submodel Service Specification - Value
Profile (SSP-003) and return the certificate submodel as defined in 
[Section 4.1](#41-aspect-model-business-partner-certificate).
...
#### 3.2.3 Certificate Acceptance

`POST /certificate-acceptance-notifications`

This API is used by the Data Consumer to provide feedback on the status to the Data Provider,
either accepting or rejecting the provided certificate. It is based on CloudEvents (see [CloudEvents](#cloudevents)).

The event MUST be of Event Type `org.catena-x.ccm.CertificateAcceptanceStatus.v1`.

The contents of the feedback message **MUST** be included in the `data` section of the CloudEvent, with
`data.status` set to one of the Acceptance-phase states according to the lifecycle of the certificate exchange 
as defined in [Section 2.1.3](#213-state-machine-certificate-exchange). For `REJECTED` or `ERRORED`, the data **MUST** 
include a non-empty errors array; each error has a message and an optional specifier scoping it to a certificate element 
(for example, a site BPN).

#### 3.2.4 Certificate Query

`POST /certificates/search`

The Data Provider **MUST** implement the search endpoint for certificates according to the
...
- the certificate contains a certifiedLocations with BPNL: `BPNL000000001AXS`
- and the certificate contains a certifiedLocations with BPNS: `BPNS000000001AXS`
- and the certificate contains a certifiedLocations with BPNA: `BPNA000000001AXS`
...
```
This example would return all certificates that match all the following conditions:
- the certificate contains a certifiedLocations with BPNL: `BPNS000000001AXS`
...
The usage policy is carried under the offering Dataset's `odrl:hasPolicy`; it is part of a Dataset.
The following illustrates a non-normative minimal policy offer:
...
> - Removal of the property `enclosedSites` in favor of `certifiedLocations`, which has a more complex
    ...
    >   (see [Section 4.2.4](#423-certified-locations))
...
|---------------------|----------------------|----------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `bpnl`              | 1..1                 | **MANDATORY**                                            | Legal entity the location belongs to. Enables group/matrix certificates across multiple legal entities. For the `CERTIFICATE_HOLDER` entry this is also the certificate holder.                                                                                                                                                                                                                                                            |
...
| `bpns`              | 0..1                 | **MANDATORY**, if assigned and matches certificate scope | Site the BPNA is assigned to in the Golden Record. Not printed on the certificate, therefore not document-verifiable; taken over from the Golden Record (rule 4.).                                                                                                                                                                                                                                                                         |
| `locationRole`      | 1..1                 | —                                                        | Role as stated on the certificate. **MUST** be one of the following: `CERTIFICATE_HOLDER` (Represents the certificate holder as stated on the certificate’s first page, including the full scope (areaOfApplication) of the certificate.), `ASSOCIATED_LOCATION` (Aggregates all additional locations without further differentiation.)                                                                                                    |                                                                                                                                                                                                                                                                                                                                                                      |                                                                                                                                                                                                                                                                                                   |
...
1. Exactly one entry in certifiedLocations **MUST** have locationRole = CERTIFICATE_HOLDER.
2. The certificate holder is unambiguously derived as certifiedLocations[locationRole="CERTIFICATE_HOLDER"].bpnl.
...
4. BPNS **MUST** be provided only if:
   1. a BPNS exists in the Golden Record AND
   2. it organizationally corresponds to the certified unit or function.
   
   Otherwise, BPNS **MUST** be omitted.
...
8. Interpretation logic (scope inheritance to subordinate addresses, hierarchy expansion)
   is explicitly out of scope of this standard and left to the Data Consumer.

#### 4.2.4 VALIDITY

The attribute `validFrom` refers to the date from which the certificate is valid.
If it is not defined, it is recommended to use the date of issue/signature of the document.
In connection with the`validFrom` date, there is the `validUntil` date for a certificate - 31.12.9999 for no expiration
date.
...
- a `createdAt` timestamp that states when the document was created,
...
Distinguish between the different identifiers:
- `exchangeId` identifies a specific exchange interaction.
- `certificateId` identifies one logical certificate identity (technical dataset identifier).
- `documentId` identifies one certificate document, usually the issued PDF (UUID).
- `revision` identifies the version of the structured metadata for that document. Acts like a change counter on the 
   logical certificate identity (metadata and document data).

The following logic **MUST** be followed:
- If only the `exchangeId` changes, the same certificate data was exchanged again. No certificate update is implied.
- If the `revision` changes while `certificateId` and `documentId` remain the same, only metadata was corrected or 
  enriched without changing the document. This still relates to the same logical certificate identity.
- If the `documentId` changes, a new certificate document was provided and must be processed again.
  The `revision` **MUST** be incremented accordingly.
- If the `certificateId` changes, this represents a new logical certificate identity.

The `revision` **MUST** be incremented for any change in the certificate metadata or document.
