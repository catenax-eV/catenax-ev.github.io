...
| [[CX-0151]](#61-normative-references) | Industry Core Basics                 | 1.1.0       |
| [[CX-0152]](#61-normative-references) | Policy Constraints for Data Exchange | 1.1.0       |
...
| **Number**                            | **Standard**                                          | **Version** |
| ------------------------------------- | ----------------------------------------------------- | ----------- |
| [[CX-0128]](#61-normative-references) | Demand and Capacity Management Data Exchange          | 2.3.1       |
| [[CX-0157]](#61-normative-references) | Predictive Unit Real-Time Information Service (PURIS) | 1.0.0       |
...
This standard provides a the following operations that are covered in the [open api definition](./assets/DemandAndCapacityNotification-open-api-samm-3.0.0.yaml):
...
This [open api definition](./assets/DemandAndCapacityNotification-open-api-samm-3.0.0.yaml) provides the full specification of the endpoints each receiving notifications consisting of a header and a payload.
...
| properties.**\<https://w3id.org/catenax/ontology/common#version\>** | The version of the standard defining the implemented API.                                        | must correspond to the version of the standard defining the "Demand and Capacity Notification API". The value must be set to `"3.0"` for APIs implementing this standard.                            |
...
##### 4.1.3.1 BACKWARD COMPATIBILITY

>
> *This section is normative*
>

For backward compatibility between certifications of standard versions `3.0.0` and `2.0.0`, a data consumer MUST additionally offer the `2.0.0` version of the API. The `2.0.0` version is deprecated with `26.09` and its support MUST end with the Ecosystem Release corresponding to `27.09`.

During this transition period the following applies:

1. The provider MUST expose exactly one data asset per supported version, differentiated solely by `cx-common:version` ("2.0" and "3.0"). This is the permitted exception to the single-asset rule in [Chapter 4.1.3](#413-data-asset-structure).
2. The `2.0.0` asset MUST use the asset definition below, retaining the previous purl context (with http in http://purl.org/dc/terms/) and `"cx-common:version": "2.0"`.
3. The `2.0.0` asset MUST reference the OpenAPI definition v2.0.0 (see [open-api](./assets/DemandAndCapacityNotification-open-api-samm-2.0.0.yaml)). This definition still contains the property `demandAndCapacityNotification` in object `content` and MUST NOT be used for new implementations.
4. Requesters MUST be able to handle multiple assets for this endpoint and MUST choose the highest compatible version (per [Chapter 4.1.3](#413-data-asset-structure)). If no compatible version exists, the requester MUST terminate the data transfer.
